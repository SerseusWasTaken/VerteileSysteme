...
object Guard {
  ...
  def apply(): Behavior[Nothing] = {
    Behaviors.setup[Receptionist.Listing] { context =>
      ...
      val region = StoreShard.createShard(context.system)

      //val ref = sharding.entityRefFor(StoreShard.TypeKey, "")
      region ! ShardingEnvelope("storeShards",StoreShard.PrintName())
